...
# All user options are controlled through config.R
source("config.R")

######  Data Preparation (if needed)  ######

# If run_data_prep is TRUE, regenerate the base data from the database
if (run_data_prep) {
  message("Starting data preparation from database...")
  
  # Load database connection libraries
  library(DBI)
  library(dbplyr)
  library(RPostgreSQL)
  
  # Source the data prep script
  source("R/0.1_Data_Prepping.R")
  
  # Run the data prep function
  # NOTE: This assumes you have a valid database connection (con)
  # If you don't have a connection set up, comment out run_data_prep <- TRUE in config.R
  prep_smc_data(con)
  
  message("Data preparation completed. Base_Data.RData has been updated.")
} else {
  message("Skipping data preparation. Using existing Base_Data.RData file.")
}

######  Main Analysis (if enabled)  ######

# If run_analysis is TRUE, proceed with the main RSCA analysis
if (run_analysis) {
  message("Starting RSCA analysis...")
  
  # Load test site data
  import_sites <- read.csv(import_sites_path)
  ...
  if (exists("sites_missing_csci") && is.data.frame(sites_missing_csci) && nrow(sites_missing_csci) > 0) {
    ...
    message(
      "The list of sites with no CSCI data has been saved to: ",
...
for (chunk_idx in seq(chunk_start, length(site_chunks))) {
  ...
  current_chunk_sites <- site_chunks[[chunk_idx]]
  # IMPORTANT: Update my_input_sites to only contain the current chunk
  my_input_sites <- current_chunk_sites
  print(paste("Processing chunk", chunk_idx, "of", length(site_chunks), "with", length(current_chunk_sites), "sites."))
  ...
  monitoring_recs <- inv %>% select(
    test_site, analytename, n_comp_samples, n_test_samples, module, n_test_samples_module,
  ...
  module_summary <- LOE_mod_sum_df %>%
    select(
      test_site, comid, latitude, longitude, sampledate, fieldreplicate, collectionmethodcode,
  ...
  ref_con_comp <- RCC_LOE_df %>%
    select(
      module, direction, test_site, sampledate, fieldreplicate, collectionmethodcode, analytename,
  ...
  stress_resp_sum <- SR_log_LOE_df %>%
    select(
      module, direction, analytename, test_site, sampledate, fieldreplicate, collectionmethodcode,
  ...
  spatial_co_sum <- SCO_LOE_df %>%
    select(
      module, direction, test_site, sampledate, fieldreplicate, collectionmethodcode, csci, analytename,
  ...
  comp_site_data2 <- Test_Comp_df %>%
    rename(
      comparator_site = comp_site, comparator_lat = comp_lat, comparator_long = comp_long,
  ...
  # Generate Excel sheets for each site in the CURRENT CHUNK ONLY
  for (i in current_chunk_sites) {
    ...
    Level3_list <- list(
      "SCO_dat_df", "SCO_LOE_df", "SR_log_dat_df",
...
if (merge_csvs) {
  ...
  message("Skipping RSCA analysis. Set run_analysis <- TRUE in config.R to run the analysis.")
}
